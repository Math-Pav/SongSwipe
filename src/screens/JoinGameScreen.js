...
const SERVER_URL = 'ws://141.145.220.13:3001';
...
export default function JoinGameScreen({ navigation }) {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      console.log('Connected, joining room:', roomCode.toUpperCase().trim());
      wsService.joinRoom(roomCode.toUpperCase().trim(), pseudo.trim());
    } catch (error) {
      setLoading(false);
      console.log('Erreur connexion:', error);
      Alert.alert('Erreur de connexion', 'Vérifie que le serveur est lancé');
    }
  };
